...
`mailsort` connects to any IMAP mail server, trains a machine learning model on the folders (IMAP labels) you have
already assigned to your emails, and uses that model to suggest or apply folders to new messages - without ever
having to read the content of your emails. It has no dependency on Google APIs, so it works with any mail provider
that offers plain IMAP access, for example with an app password. If you are specifically looking for Gmail support
(OAuth, the Gmail label API, a sorting daemon and a web UI) see
`gmailsorter <https://github.com/jan-janssen/gmailsorter>`_, which depends on `mailsort` for the shared IMAP and
machine learning core.

`mailsort` is deliberately a plain Python library and command line tool rather than a hosted service: you run it
yourself, on your own schedule (for example from cron), against your own local database.
